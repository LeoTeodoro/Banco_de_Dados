...
#JOIN EM UM AUTORELACIONAMENTO

SELECT e.Pnome AS 'Nome Empregado',s.Pnome AS 'Nome Supervisor'
FROM empregado AS e, empregado AS s
WHERE e.SSN_Supervisor = s.SSN;

#JOIN COM 3 TABELAS

SELECT e.PNome AS 'Nome Empregado', p.Pnome AS 'Nome Projeto'
FROM empregado AS e, projeto AS p, trabalha_em AS te
WHERE te.SSN_Empregado = e.SSN AND te.PNumero_Projeto = p.PNumero;

#RETIRANDO RESULTADOS REPETIDOS
